		...
		while (currentNode != this._endNode) {
			...
			for (var i: number = startX; i <= endX; i++) {
				...
				for (var j: number = startY; j <= endY; j++) {
					...
					if (test == currentNode || !test.runable||!this._grid._nodes[currentNode.x][test.y].runable||!this._grid._nodes[test.x][currentNode.y].runable)
						{continue;}
...
//return this._closedList.indexOf(node) > 0 ? true : false;
	...
	public validNode(node: TileNode, currentNode: TileNode): Boolean {
		if (currentNode == node || !node.runable) return false;

		if (!this._grid._nodes[currentNode.x][node.y].runable) return false;

		if (!this._grid._nodes[node.x][currentNode.y].runable) return false;
